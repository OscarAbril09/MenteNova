...
internal_cjs.config.COLLAPSIBLE_WIDTH_THRESHOLD = 1200;
let contexts = [];
let undoFuncs = [];
function contextInit(context) {
    ...
    context.calendarApi.on('_unmount', () => {
        internal_cjs.removeExact(contexts, context);
        if (!contexts.length) {
...
function freezeScrollgridWidths() {
    let els = internal_cjs.findElements(document.body, '.fc-scrollgrid');
...
function freezeScrollGridWidth(el) {
    ...
    if (!el.classList.contains('fc-scrollgrid-collapsible') || elWidth < internal_cjs.config.COLLAPSIBLE_WIDTH_THRESHOLD) {
